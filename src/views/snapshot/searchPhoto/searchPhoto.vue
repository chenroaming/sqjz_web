<template>
    <div>
        <!-- <Imagelamp :isShow="lampIsShow" @closeModal="lampIsShow=false" :path="lPhotoPath"/>
            <Searcharea v-model="searchData" :visible="false" @handleSearch="handleSearch" @refreshData="refreshData" placeholder="请输入搜索内容"> -->
        <div slot="extraArea"
            style="white-space:nowrap;height:50px;line-height:50px;text-align:right;padding-right:30px;">
            <!-- <el-select v-model="camSelectStr" placeholder="请选择摄像头列表" style="margin:0 5px 0 10px">
                <el-option label="摄像头列表(全部)" value=""></el-option>
                <el-option
              ...
              <el-select v-model="nameSelectStr" placeholder="请选择名单列表" style="margin:0 5px 0 10px">
                <el-option
                  v-for="item in nameListOptions"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id">
                </el-option>
        ...
        <!-- </Searcharea> -->
        <!-- <el-table v-loading="isLoading" class="snapTable" height="250px" :data="tableData" ref="snapTableRef"
            style="width: 100%">
            <el-table-column label="抓拍人脸" align="center" width="180">
                <template slot-scope="scope">
                    <img :src="scope.row.rPhoto" alt="图片" style='width:100px;height:100px;cursor:pointer'
                        @click="handleLoogBigPic(scope.row.lPhoto)">
                </template>
            </el-table-column>
            <el-table-column prop="similarity" align="center" label="相似度">
                <template slot-scope="scope">
                    {{ scope.row.similarity | similarityPercent }}
                </template>
            </el-table-column>
            <el-table-column label="标准人脸" align="center" width="180">
                <template slot-scope="scope">
                    <img :src="scope.row.uPhoto? scope.row.uPhoto : require('@/assets/icons/defaultuser.jpg')"
                        alt="暂无图片" style='width:100px;height:100px'>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="webcamName" label="抓拍位置" align="center" width="180">
            </el-table-column>
            ...
            <!-- <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <el-tag v-if="scope.row.gender === '1'" type="success">女</el-tag>
              </template>
            </el-table-column> -->
        ...
        <!-- </el-table> -->
        <!-- <Sortpage :totalPages="totalPages" @sizeChange="handleSizeChange" :currentPage="currentPage"
            :sortShow="sortShow" /> -->
        <Findsearch @submitSuccess="handleUploadSuccess" @closeModal="uploadVisible = false"
            :dialogVisible="uploadVisible" />
        <Facetrace :authDrawerVisible="faceTraceDrawerVisible" @closeModal="faceTraceDrawerVisible=false"
...
<script>
    ...
    export default {
        ...
        methods: {
            __Init(pageSize = 10, pageNumber = this.pageNumber, category, webcamId, faceId = this.faceId, startDate = '', endDate = '', name = this.searchName) {
                this.isLoading = true;
                getSnapShotList(pageSize, pageNumber, category, webcamId, faceId, startDate, endDate, name)
                    .then(res => {
            ...
            handleSizeChange(num) {
                ...
                this.__Init(10, num, this.nameSelectStr, this.camSelectStr, this.faceId, this.searchDate[0], this.searchDate[1]);
            },

            // 搜索
            handleSearch() {
                this.__Init(10, 1, this.nameSelectStr, this.camSelectStr, '', this.searchDate[0], this.searchDate[1], this.searchName);
            },
