    ...
    <el-scrollbar class="scrollbar">
      ...
      <el-table v-loading="isLoading" :data="tableData" :header-cell-style="rowClass" class="tableShadow">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button v-permission="['role:operate']" type="primary" size="mini" @click="handleRole('UPDATE_AUTH', scope.row)">权限配置</el-button>
            <el-button v-permission="['role:operate']" type="danger" size="mini" @click="handleRole('DELETE_ROLE', scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-scrollbar>
  ...
  methods: {
    __init(pageSize = 10, pageNumber = 1) {
      getRoleList2(pageSize, pageNumber)
        .then(({ data: { pageNumber, list, total, state }}) => {
          if (state === '100') {
