...
<template>
  <div>
    <div style="margin: 10px;padding:30px;border-bottom:2px solid #009df7">
      <el-input v-model="productName" style="width: 180px;" placeholder="请输入产品名称"/>
      <el-input v-model="name" style="width: 180px;" placeholder="请输入矫正对象姓名"/>
    ...
    <div style="padding: 10px 30px;">
      ...
      <el-row :gutter="10" style="margin-top: 20px;">
        <transition-group name="el-fade-in-linear">
          <el-col v-for="(item2,index2) in tableData" :span="4" :key="item2.productId">
            <el-card style="width: 100%;height: 450px;">
              <el-image :title="item2.productName" :src="item2.productPicPaths[0]" style="cursor: pointer;height: 250px;" @click="showDetail(item2)"/>
              <div style="padding: 14px;">
                <span :title="item2.productName" class="title">{{ item2.productName }}</span>
                <p class="price">
    ...
    <showDetail ref="showDetail" :product-obj="productObj"/>
    <audit ref="audit" :product-id="productId" @handleSuccess="getList"/>
  </div>
</template>
