  ...
  <div>
    <div class='searchArea'>
      <div class='backTo' style="cursor:pointer">
        ...
        <span style="margin-right:8px;"  @click="selectState('')">总人数：<span>{{signed+absent}}</span></span>
    ...
    <el-table
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <img :src="scope.row.rSignInPhoto" alt="" style="width:80px;height:100px;cursor:pointer" @click="handleLookLphoto(scope.row.lSignInPhoto)">
          <p style="width:100%;cursor:pointer;text-align:center;font-size:12px">{{ scope.row.signInDate == null?'':scope.row.signInDate.time | formatTime}}</p>
        </template>
      </el-table-column>
      <el-table-column
        label="签退记录"
        align="center">
        <template slot-scope="scope">
          <img :src="scope.row.rSignOutPhoto" alt="" style="width:80px;height:100px;cursor:pointer" @click="handleLookLphoto(scope.row.lSignOutPhoto)">
          <p style="width:100%;cursor:pointer;text-align:center;font-size:12px">{{ scope.row.signOutDate == null?'':scope.row.signOutDate.time | formatTime}}</p>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        align="center">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.iastate === 0">签到</el-tag>
          <el-tag type="danger" v-if="scope.row.iastate === 2">缺席</el-tag>
        </template>
      </el-table-column> 
    </el-table>
    <!-- 查看访客详情 -->
...
<script>
import { getAreaDetail, getCheckon, getTaskFaceset,getTaskDate,findDeleteRecord} from '@/api/attendance';
...
export default {
  ...
  methods: {
    ...
    handleBack () {
      ...
      this.$router.push({name:'area-manger'});
    },
