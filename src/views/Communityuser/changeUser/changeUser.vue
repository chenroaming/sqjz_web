<script>
  ...
  import { validatePassword,validateForbiChar,validatePhoneNumber,validateIdCard } from "@/utils/validate";
  // 身份证手机号校验
  export default {
    filters: {},
    props: {
    ...
    data() {
      ...
      return {
        isLoading:false,
        userType: [{ id: 1, value: "宽管" },{ id: 2, value: "普管" },{ id: 3, value: "严管" }],
        userType2: ['宽管','普管','严管'],
        activityRange: [{ id: 1, value: "区/县" },{ id: 2, value: "市" }],
        userFileList:[{ fileType:1,fileText:'基本信息表' },{ fileType:2,fileText:'定位监管承诺书' },{ fileType:3,fileText:'告知书' },{ fileType:4,fileText:'宣告书' },{ fileType:5,fileText:'责任书' },{ fileType:6,fileText:'软件监管承诺书' },],
        imageUrl:'',
        userInfo: {
          activityRange: "",
          location:'',
...
<template>
  <div>
    <router-link :to="{ path:'/Communityuser/index'}">
      ...
      <el-row :gutter="20">
        <el-col :span="3">
          <el-upload
            ...
            <img v-if="imageUrl" :src="imageUrl" class="avatar" style="width:170px;height:200px">
            <i v-else class="el-icon-plus avatar-uploader-icon"></i>
          </el-upload>
        </el-col>
        <el-col :span="13">
          <el-form ref="userInforefs" :rules="rules" :model="userInfo" size="mini" label-position="right" label-width="120px">
            <el-form-item label="姓名" prop="name" required class="formItem">
              <el-input v-model="userInfo.name" class="formInput" placeholder="请输入姓名" />
            </el-form-item>
            <el-form-item label="身份证号" prop="identityCard" required class="formItem">
              <el-input v-model="userInfo.identityCard" class="formInput" placeholder="请输入身份证号" />
            </el-form-item>
            <el-form-item label="手机号码" prop="phone" required class="formItem">
              <el-input v-model="userInfo.phone" class="formInput" placeholder="请输入手机号码" />
            </el-form-item>
            <el-form-item label="人员类型" prop="userType" required class="formItem">
              <el-select v-model="userInfo.userType" class="formInput" placeholder="人员类型">
                <el-option v-for="(item, index) in userType" :key="item.id" :label="item.value" :value="item.id"/>
              </el-select>
            </el-form-item>
            <el-form-item label="活动范围" prop="activityRange" required class="formItem">
              <el-select v-model="userInfo.activityRange" class="formInput" placeholder="活动范围">
                <el-option v-for="(item, index) in activityRange" :key="index" :label="item.value" :value="item.id"/>
              </el-select>
            </el-form-item>
            <el-form-item label="案由" prop="cause" required class="formItem">
              <el-input v-model="userInfo.cause" class="formInput" placeholder="请输入案由" />
            </el-form-item>
            <el-form-item label="居住小区" prop="livingArea" required class="formItem">
              <el-input v-model="userInfo.livingArea" class="formInput" placeholder="居住小区"></el-input>
            </el-form-item>
            <el-form-item label="备注" prop="remark" class="formItem">
              <el-input v-model="userInfo.remark" class="formInput" placeholder="备注"></el-input>
            </el-form-item>
            <el-form-item label="居住位置" required class="formItem">
              ...
              <el-button type="primary" @click="showMap = true">修改区域坐标</el-button>
            </el-form-item>
            <el-form-item class="formItem">
              <el-button type="success" @click="submitForm('userInforefs')" :loading="isLoading">提 交</el-button>
            </el-form-item>
          </el-form>
        </el-col>
        <el-col :span="8">
    ...
    <el-card class="box-card" style="margin-top: 10px;">
      ...
      <userFile v-for="(item,index) in userFileList" :key="index" ref="userFile" :userId="userInfo.userId" :userFiles="userInfo.userFiles" :fileType="item.fileType" :fileText="item.fileText"></userFile>
    </el-card>
  </div>
</template>
