<template>
  <div class="pm-distribution">
    <Imagelamp :is-show="lampIsShow" :path="lPhotoPath" @closeModal="lampIsShow=false"/>
    <h3 class="title">抓拍地图分布</h3>
    <div id="container" :style="{height:containerHeight}" class="container">
      <div class="left">
        <div style="width:100%">
          <el-input v-model="searchName" placeholder="请输入您要搜索的人名">
        ...
        <div :style="{height:leftHeight}" class="list">

          <div class="list-context">
            <p>共 <span style="color:red;font-weight:bold"> {{ markers.length }} </span> 个抓拍记录</p>
            <div ref="box" :style="{height:leftHeight}" class="list-scroll bmr-y-scroll">
              <!-- 项目列表-->
              <div class="listItemDIV">
                <div v-for="(marker,index) of markers" :key="marker.id+index" :class="{active: activeName == marker.id+index}" class="row" @click="lookDetail(marker,'left',index)">
                  <div
                    :title="marker.name"
                    class="head-title"
                    v-text="marker.name"/>
                  <div class="row-content">
                    <span class="item"><i class="iconfont nhjc-dianli electricity"/>抓拍相机：{{ marker.webcamName }}</span>
                    <span class="item"><i class="iconfont nhjc-shui water"/>抓拍时间：{{ marker.time.time | formatTime }}</span>
                  </div>
                </div>
      ...
      <div id="right-context" class="right-context">
        <baidu-map id="mapBox" :zoom="map.zoom" :center="{lng: map.center.lng, lat: map.center.lat}" style="width:100%;height:100%" class="map" ak="DvoyV7vtMIgqmpM0UM7Xg0Yd0MyUyvi8" @ready="handler">
          <!--比例尺控件-->
          ...
          <bm-marker-clusterer :average-center="true">
            <bm-marker v-for="(marker,index) in markers" :key="marker.id+index" :position="{lng: marker.lng, lat: marker.lat}" @click="lookDetail(marker,index)">
              <bm-label :content="index+1+''" :label-style="labelStyle" :offset="offset"/>
            </bm-marker>
          </bm-marker-clusterer>
          <!--信息窗体-->
          <bm-info-window :position="{lng: infoWindow.info.lng, lat: infoWindow.info.lat}" :title="infoWindow.info.name" :show="infoWindow.show" @close="infoWindowClose" @open="infoWindowOpen" >
            ...
            <p><img :src="infoWindow.info.rPhoto" class="right" style="width:80px;height:100px;cursor:pointer;" @click="handleLoogBigPic(infoWindow.info.lPhoto)"></p>
    ...
    <div v-show="nameList.length>0" class="block nameList">
        ...
        .then(res => {
          ...
          if (this.markers.length > 1) {
            ...
            this.markers.forEach(item => {
              const aObj = new BMap.Point(item.lng, item.lat)
              pois.push(aObj)
            })
            // console.log(pois)
