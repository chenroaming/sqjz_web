<template>
  <div v-if="addInfoIsShow" :class="addInfoIsShow===true? 'fadeIn' : ''" class="faceWrap-mask" style="z-index:1000">
    <div class="faceWrap" >
      <div class="faceWrap-header">
        <el-button icon="el-icon-close" type="primary" circle size="mini" style="text-align:right" @click="$emit('closeWindow');searchName='';radio = '1'"/>
      </div>
      <div class="faceWrap-body">
        ...
        <el-input v-model="searchName" placeholder="请输入姓名查找" style="margin-left:38%;width:150px;margin-bottom:20px" @change="searchList(searchName)"/>
        <el-table
          ref="multipleTable"
          :data="tableData"
          tooltip-effect="dark"
        ...
        <Sortpage :total-pages="totalPages" :page-size="pageSize" class="faceSortPage" @sizeChange="sizeChange"/>
      </div>
      <div v-if="radio==2" class="faceWrap-footer">
        <el-button v-if="tableData.length !=0" type="primary" @click="addAll">添加全部</el-button>
        <el-button type="primary" @click="submitFaceList">添加人脸到设备</el-button>
      </div>
      <div v-if="radio==1" class="faceWrap-footer">
        <el-button v-if="tableData.length !=0" type="primary" @click="addAll">添加全部</el-button>
